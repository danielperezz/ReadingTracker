...
#TODO: fix: when converting to exe, the set date window is blank
# ~~~~~~~~~~~~ functions ~~~~~~~~~~~~~~~ #
def add_book():
    ...
    if year=="Pick a Year":
        ...
        for record in mycursor:
            ...
            if record[5] is not None:
                rank_sum += record[5]
        if not total_finished:  # if there are no books finished
            stat_info_label.config(text="No books finished in {}".format(year))
        else:
            average_time = delta_sum/total_finished
            average_rank = rank_sum/total_finished
            stat_info_label.config(text="Number of books finished in {}: \n\n"
                                        "Average number of days to finish book in {}:\n\n"
                                        "Average rank per book in {}:"
                                   .format(year, year, year))
            stat_result_label.config(text="{}\n\n{:.2f}\n\n{:.2f}".format(result[0], average_time, average_rank))
...
switch_to_stat.bind("<Button-1>", open_stat_frame)
...
years = ttk.Combobox(stat_frame, values=["2021", "2022"], state="readonly")
years.set("Pick a Year")
